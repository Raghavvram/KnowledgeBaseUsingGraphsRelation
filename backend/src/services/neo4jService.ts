...
class Neo4jService {
  ...
  private async connect() {
    ...
    const username = process.env.NEO4J_USER || 'neo4j';
            ...
            console.log(`Attempting to connect to Neo4j at URI: ${uri} with user: ${username}`);
    
            const maxAttempts = parseInt(process.env.NEO4J_CONNECT_ATTEMPTS || '6', 10);
            const delayMs = parseInt(process.env.NEO4J_CONNECT_DELAY_MS || '5000', 10);
        ...
        try {
          // race with a short timeout to avoid hanging forever
          await Promise.race([
          ...
          this.connected = true;
          console.log('\u2705 Neo4j connection established (attempt ' + attempt + ')');
          return;
        } catch (err) {
          await session.close();
          throw err;
        ...
        console.error(`\u274c Neo4j connection attempt ${attempt} failed:`, error && (error as any).message ? (error as any).message : error);
        if (attempt < maxAttempts) {
          console.log(`Waiting ${delayMs}ms before next Neo4j connect attempt...`);
          // ensure driver is closed before retrying
          try { if (this.driver) await this.driver.close(); } catch (e) {}
        ...
        console.log('2. Check if Neo4j is accessible at:', uri);
